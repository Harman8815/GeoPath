...
export const AlgorithmCards: React.FC<AlgorithmCardsProps> = ({
  onSelectAlgorithmForPlayground,
  onOpenMap,
}) => {
  const [activeSimulation, setActiveSimulation] =
    useState<AlgorithmType | null>("astar");
      ...
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4 mb-12">
          ...
          <p className="text-slate-400 text-sm sm:text-base leading-relaxed">
            From classic greedy exploration to heuristic spatial optimization,
            GeoPath provides precise visual feedback on how each algorithm
            ...
            {ALGORITHMS_DATA.map((algo) => {
              ...
              return (
                <SwiperSlide key={algo.id} className="algorithm-slide">
                  <div
                    className={`relative group rounded-2xl p-6 bg-[#16181d]/90 border backdrop-blur-xl transition-all duration-500 flex flex-col justify-between w-[520px] h-[620px] ${
                      isSimulating
                  ...
                  >
                      ...
                      <div className="space-y-1.5 mb-6">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {algo.useCases.map((useCase, idx) => (
                            <span
                              key={idx}
                              className="text-xs bg-slate-900 text-slate-300 px-2.5 py-1 rounded-md border border-slate-800"
                            >
                              • {useCase}
                            </span>
                          ))}
                    ...
                    <div className="pt-4 border-t border-slate-800 flex items-center justify-between gap-3">
                      <button
                        onClick={() =>
                          setActiveSimulation(isSimulating ? null : algo.id)
                        }
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() =>
                            onSelectAlgorithmForPlayground(algo.id)
                          }
